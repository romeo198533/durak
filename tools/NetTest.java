 ...
 *
 * Гоняет партии, где глава стола и оба игрока сидят в одном процессе, а ходы
 * ходят по-настоящему — строкой через {@link Wire}, как поедут по Блютусу.
 * Оба игрока — программы, и каждая решает по одному только своему виду места:
 * другого у неё и нет. Этим и проверяется главное — что вида хватает, чтобы
 * играть, и что чужого в нём не едет.
 *
 * Запуск:
 *
